    ...
    return (
        <div className={`bg-white border border-gray-400 shadow-sm rounded-industrial-sm p-4 flex items-center justify-between gap-4 ${className}`}>
            <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center border ${hasPolygon ? 'bg-white border border-gray-400 shadow-sm border-gray-400 shadow-sm text-brand-navy-bright' : 'bg-red-500/10 border-red-500/30 text-red-500'}`}>
            ...
            <div className="text-right">
                <p className="text-[9px] text-brand-navy uppercase font-bold mb-1">Coordenadas Base</p>
                <p className="text-xs font-mono font-bold text-brand-navy ">{coords}</p>
            </div>
        </div>
    );
}
