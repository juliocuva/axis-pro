...
export default function PublicCVAPage() {
    ...
    const handleGenerateLot = async (e: React.FormEvent) => {
        ...
        try {
            // Generate a unique Lot ID for CVA
            const dateStr = new Date().toISOString().slice(0, 7).replace('-', ''); // YYYYMM
            const randomCode = Math.floor(1000 + Math.random() * 9000);
            const lotId = `CVA-${dateStr}-${randomCode}`;

            // 1. Create a dummy inventory record for the lead to maintain traceability
            const { data: invData, error: invError } = await supabase.from('coffee_purchase_inventory').insert({
                ...
                purchase_weight: 0,
                status: 'cupping_completed',
                process_data: {
                    lead_phone: leadPhone,
                    source: 'public_landing_page_cva'
                }
            }).select().single();
            ...
            // 2. Save the CVA assessment data tied to the dummy inventory record
            const { error: cvaError } = await supabase.from('sca_cupping').insert({
                inventory_id: inventoryId,
                company_id: 'PUBLIC_LEAD',
                cva_descriptive: cvaData.cva_descriptive,
                cva_affective: cvaData.cva_affective,
                notes: cvaData.notes,
                taster_name: cvaData.taster_name || leadName
            });

            if (cvaError) throw cvaError;

            setSuccessLotId(lotId);
        } catch (err) {
            console.error("Error generating public lot:", err);
            alert("Hubo un error generando el reporte CVA. Por favor intenta de nuevo.");
        } finally {
        ...
        return (
            <div className="min-h-screen bg-brand-navy flex flex-col items-center justify-center p-6 text-white text-center">
                <div className="bg-[#0b1727] p-10 rounded-3xl border border-brand-green/20 max-w-lg w-full shadow-2xl">
                    ...
                    <h2 className="text-3xl font-black mb-4">Reporte CVA Asegurado</h2>
                    <p className="text-white/70 mb-8 font-medium">
                        Tus datos de catación han sido procesados. Se te ha asignado el siguiente código de lote único en nuestro sistema:
                    </p>
                    ...
                    <p className="text-sm text-white/50 mb-8">
                        Guarda este ID. Te servirá más adelante para conectarte con Axis One Coffee y anexar más información a este mismo lote.
                        <br/><br/>
            ...
            <main className="max-w-5xl mx-auto px-6 py-12">
                <div className="mb-12 text-center max-w-2xl mx-auto">
                    ...
                    <p className="text-gray-600 font-medium">
                        Registra perfiles sensoriales, afectivos y extrínsecos según el nuevo estándar SCA CVA.
                    </p>
                </div>

                <div className="bg-white shadow-xl border border-gray-100 rounded-3xl overflow-hidden">
                    <CVAAssessmentForm
                        inventoryId="" // Ignored in public mode
                        isPublic={true}
                ...
                <div className="fixed inset-0 z-[100] bg-brand-navy/90 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl p-8 md:p-10 max-w-md w-full shadow-2xl relative animate-in zoom-in-95 duration-300">
                        ...
                        <p className="text-sm text-gray-500 mb-8 font-medium">
                            Para generar tu reporte CVA con Hash Criptográfico y asegurar estos datos, regístrate a continuación.
                        </p>
