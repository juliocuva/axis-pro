...
export default function AuthScreen({ onLogin }: AuthScreenProps) {
    ...
    const steps = [
        ...
        { id: 'step-07', label: '07', title: language === 'en' ? 'Vision' : 'Visión' },
        { id: 'step-08', label: '08', title: language === 'en' ? 'Join' : 'Unirse' },
        { id: 'step-09', label: '09', title: language === 'en' ? 'Get in Touch' : 'Contacto' },
    ];
    ...
    return (
        <div className="min-h-screen bg-soft-white text-brand-navy selection:bg-brand-green selection:text-white font-sans font-medium">
            ...
            <main id="step-01" className="relative pt-32 pb-16 px-8 overflow-hidden">
                ...
                <div className="max-w-6xl mx-auto text-center space-y-8 relative z-10">
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                        ...
                        <h1 className="text-6xl lg:text-7xl font-black leading-[0.95] max-w-4xl mx-auto text-brand-navy tracking-tighter uppercase">
                            {t.hero.headline.split(' ').map((word, i) => i === t.hero.headline.split(' ').length - 1 ? <span key={i} className="text-brand-green">{word}</span> : word + ' ')}
                        </h1>
                        <p className="text-base text-brand-navy/80 font-medium max-w-3xl mx-auto leading-relaxed">
                            {t.hero.subheadline}
                        </p>
                    </div>
            ...
            {/* 4. EL CONCEPTO */}
            <section id="step-03" className="relative py-32 my-12 bg-soft-white overflow-hidden">
                <div className="absolute left-1/2 top-0 bottom-0 w-px bg-brand-green/20 hidden lg:block -translate-x-1/2 z-0"></div>
                ...
                <div className="max-w-7xl mx-auto px-8 text-center relative z-10">
                    ...
                    <h2 className="text-[50px] font-black uppercase text-brand-navy tracking-tighter mb-12 leading-none">{t.concept.headline}</h2>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-4 mb-24">
                        {t.concept.list.map((item, i) => (
                            <div key={i} className="group p-6 bg-white border border-brand-gray/50 rounded-full aspect-square flex flex-col items-center justify-center text-center hover:border-brand-green/30 transition-all hover:shadow-xl hover:shadow-brand-green/5 space-y-4">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-tight text-brand-navy/60 group-hover:text-brand-navy transition-colors max-w-[80px] text-center leading-none">{item}</span>
                            </div>
                        ))}
                    </div>

                    {/* SOLUTION PHRASE - CENTERED BOTTOM */}
                    <div className="pt-16 border-t border-brand-gray/50">
                        <p className="text-2xl md:text-3xl font-black text-brand-green uppercase tracking-tighter max-w-4xl mx-auto leading-none">
            ...
            {/* 6. PARA QUIÉN */}
            <section id="step-05" className="relative py-32 my-12 bg-soft-white border-y border-brand-gray/50">
                <div className="absolute left-1/2 top-0 bottom-0 w-px bg-brand-green/20 hidden lg:block -translate-x-1/2 z-0"></div>
                ...
                <div className="max-w-7xl mx-auto px-8 relative z-10 text-center">
                    <div className="mb-24 space-y-4 text-center">
                        ...
                        <h2 className="text-[50px] font-black uppercase text-brand-navy tracking-tighter max-w-4xl mx-auto leading-none">{t.target.headline}</h2>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-32">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
                            {t.target.list.map((item, i) => (
                                <div key={i} className="group p-6 bg-white border border-brand-gray/50 rounded-full aspect-square hover:border-brand-green/30 transition-all hover:shadow-lg hover:shadow-brand-green/5 flex flex-col items-center justify-center gap-4">
                                    ...
                                    <span className="text-[12px] font-black uppercase tracking-widest text-brand-navy/60 group-hover:text-brand-navy transition-colors max-w-[90px] text-center leading-tight">{item}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="max-w-7xl mx-auto px-8 text-center relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-32">
                        {t.pricing.tiers.map((tier, i) => (
                            <div key={i} className={`p-12 rounded-industrial border transition-all duration-500 text-left space-y-8 group ${i === 1 ? 'border-brand-green shadow-2xl shadow-brand-green/10 bg-soft-white scale-105 z-10' : 'border-brand-gray/50 bg-white hover:-translate-y-4 hover:shadow-2xl hover:shadow-black/5'}`}>
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs text-brand-navy/60 font-medium uppercase tracking-widest">{tier.desc}</p>
                                </div>
                                <div className="text-2xl font-black text-brand-green tracking-tighter">{tier.price}</div>
                                <ul className="space-y-4 pt-8 border-t border-brand-gray/50">
                                    {tier.features.map((f, j) => (
                                        <li key={j} className="flex items-center gap-3 text-[12px] font-black uppercase text-brand-navy/60 tracking-widest group-hover:text-brand-navy transition-colors">
            ...
            {/* 8. VISIÓN */}
            <section id="step-07" className="py-48 bg-soft-white relative overflow-hidden">
                <div className="max-w-4xl mx-auto px-8 text-center space-y-12 relative z-10">
                    <h2 className="text-2xl md:text-6xl font-black uppercase text-brand-navy tracking-tighter leading-[0.95]">{t.vision.headline}</h2>
                    <div className="w-32 h-1 bg-brand-green mx-auto"></div>
                    <div className="space-y-8 pt-12">
                        <p className="text-base font-bold text-brand-navy/80 uppercase tracking-tight">{t.vision.copy1}</p>
            ...
            {/* 8. ADOPTANTE TEMPRANO / JOIN */}
            <section id="step-08" className="relative py-32 bg-white border-t border-brand-gray/50 overflow-hidden">
                <div className="max-w-7xl mx-auto px-8 text-center relative z-10">
                    <div className="flex flex-col items-center gap-2 mb-12">
                        <div className="w-3 h-3 rounded-full bg-brand-green mb-2"></div>
                        <span className="text-[120px] font-thin text-brand-green leading-none tracking-tighter">08</span>
                        <span className="text-xs font-semibold text-brand-green uppercase tracking-ultra-wide">{language === 'en' ? 'Join' : 'Unirse'}</span>
                    </div>
                    <h2 className="text-2xl md:text-6xl font-black uppercase tracking-tighter leading-none max-w-5xl mx-auto mb-12">{t.footer.headline}</h2>
            ...
            {/* 9. CONTACTO / GET IN TOUCH */}
            <section id="step-09" className="relative py-32 bg-soft-white border-t border-brand-gray/50 overflow-hidden">
                {/* Visual Connector Line */}
                <div className="absolute left-1/2 top-0 bottom-0 w-px bg-brand-green/20 hidden lg:block -translate-x-1/2 z-0"></div>
                ...
                <div className="max-w-7xl mx-auto px-8 relative z-10">
                    <div className="mb-20 space-y-4 text-center max-w-4xl mx-auto">
                        <div className="flex flex-col items-center gap-2">
                            <div className="w-3 h-3 rounded-full bg-brand-green mb-2"></div>
                            <span className="text-[120px] font-thin text-brand-green leading-none tracking-tighter">09</span>
                        ...
                        <p className="text-base text-brand-navy/60 font-medium max-w-2xl mx-auto leading-relaxed mt-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
                        <div className="lg:col-span-5 space-y-6">
                            ...
                            <div className="p-6 bg-white border border-brand-green/20 rounded-industrial-sm shadow-md relative overflow-hidden group">
                                <div className="absolute top-0 right-0 bg-brand-green text-white px-3 py-1 text-[8px] font-black uppercase rounded-bl-xl tracking-widest">
                                    {language === 'en' ? 'Core Developer' : 'Desarrollador Core'}
                                </div>
                                <div className="flex items-center gap-4 mb-4">
                                    <div className="w-12 h-12 rounded-full bg-brand-green/10 flex items-center justify-center text-brand-green font-black text-lg select-none shrink-0 border border-brand-green/20">
                                        JR
                                    </div>
                                    <div>
                                        <h4 className="text-sm font-black text-brand-navy uppercase leading-tight">Julio Cesar Uva Ramirez</h4>
                                        <p className="text-[10px] text-brand-green font-black uppercase tracking-wider mt-0.5">
                                            {language === 'en' ? 'Systems Engineer & AI Developer' : 'Ingeniero de Sistemas / IA Developer'}
                                        </p>
                                    </div>
                                </div>
                                <div className="space-y-3 pt-3 border-t border-brand-gray/50 text-xs">
                                    <a 
                                        href="https://wa.me/573013970002?text=Hola%20Julio,%20me%20gustaria%20agendar%20una%20demo%20de%20AxisOne%20Coffee" 
                                        target="_blank" 
                                        rel="noopener noreferrer" 
                                        className="flex items-center gap-3 text-brand-navy/70 hover:text-brand-green transition-colors font-bold"
                                    >
                                    ...
                                    <a 
                                        ...
                                        target="_blank" 
                                        rel="noopener noreferrer" 
                                        className="flex items-center gap-3 text-brand-navy/70 hover:text-brand-green transition-colors font-bold break-all"
                                    >
                            ...
                            <div className="flex items-start gap-4 p-6 bg-white border border-brand-gray/50 rounded-industrial-sm hover:border-brand-green/30 transition-all duration-300">
                                ...
                                <div className="space-y-1">
                                    <h4 className="text-[10px] font-black uppercase text-brand-navy/40 tracking-wider">Operations Hub</h4>
                                    <span className="text-base font-bold text-brand-navy">Pereira, Colombia</span>
                                </div>
                            </div>
                        </div>

                        {/* Contact Form card */}
                        <div className="lg:col-span-7 bg-white border border-brand-gray/50 rounded-industrial p-8 md:p-12 shadow-xl shadow-black/[0.02]">
                            {isContactSubmitted ? (
                                <div className="text-center py-8 space-y-6 animate-in fade-in duration-500">
                                    ...
                                    <div className="space-y-3">
                                        <h3 className="text-2xl font-black uppercase text-brand-navy tracking-tight">{t.contact.successTitle}</h3>
                                        <p className="text-base text-brand-navy/60 font-light leading-relaxed max-w-md mx-auto">{t.contact.successSubtitle}</p>
                                    </div>
                                    <div className="pt-6">
                                        <a 
                                ...
                                <form onSubmit={handleContactSubmit} className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-brand-navy/40 uppercase tracking-widest ml-1">{t.contact.name}</label>
                                            <input 
                                                type="text" 
                                                required 
                                                value={contactName} 
                                                onChange={(e) => setContactName(e.target.value)}
                                                className="w-full border-b border-black/10 px-1 py-3 text-sm focus:border-brand-green outline-none transition-all font-bold text-brand-navy bg-transparent" 
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-brand-navy/40 uppercase tracking-widest ml-1">{t.contact.email}</label>
                                            <input 
                                                type="email" 
                                                required 
                                                value={contactEmail} 
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-brand-navy/40 uppercase tracking-widest ml-1">{t.contact.company}</label>
                                            <input 
                                                type="text" 
                                                required 
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-brand-navy/40 uppercase tracking-widest ml-1">{t.contact.phone}</label>
                                            <input 
                                                type="tel" 
                                                required 
                                                value={contactPhone} 
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-brand-navy/40 uppercase tracking-widest ml-1">{t.contact.role}</label>
                                        <select 
                                            required
                                            value={contactRole}
                                            onChange={(e) => setContactRole(e.target.value)}
                                            className="w-full border-b border-black/10 px-1 py-3 text-sm focus:border-brand-green outline-none transition-all font-bold text-brand-navy bg-transparent cursor-pointer"
                                        >
                                            <option value="" disabled className="text-gray-400">{t.contact.rolePlaceholder}</option>
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-brand-navy/40 uppercase tracking-widest ml-1">{t.contact.message}</label>
                                        <textarea 
                                            required 
                                            rows={4}
                                            value={contactMessage} 
                                            onChange={(e) => setContactMessage(e.target.value)}
                                            className="w-full border-b border-black/10 px-1 py-2 text-sm focus:border-brand-green outline-none transition-all font-bold text-brand-navy bg-transparent resize-none" 
                                        />
                                    </div>
                    ...
                    <div className="pt-24 mt-24 border-t border-brand-gray/50 text-center">
                        <img src="/logo.png" alt="AXISONE" className="h-12 mx-auto mb-6 opacity-80" />
                        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-brand-navy/40">© 2026 AXISONE COFFEE INFRASTRUCTURE COLOMBIA</p>
                    </div>
                </div>
            </section>
